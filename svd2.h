 ...
 *
 * written by Carl Edward Rasmussen.1996 
 * Modified by Li Teng 2010/10/1  */

/* input/output: 
 * float *D     data matrix to be decomposed is D[n][n] / decomposed result 
 * float *S2    empty vector / square of the singular values  
 * int n        dimension of the data matrix to be decomposed
 * int maxsize  maximum number of column of D is maxsize
 */


void svd2(double *D, double *S2, int n, int maxsize)
{
  ...
  double **A;
   ...
   A[0] = malloc( 2*n*n * sizeof(double) );
   for (i=1;i<2*n;i++)
      A[i] = A[i-1] + n; 
